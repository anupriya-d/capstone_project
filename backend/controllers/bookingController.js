...
const getAllBookings = ( res) => {
  Models.Booking.find({}).populate('user').populate('track')
...
const getBookingsByUserId = async (req, res) => {
  try {
    const reviews = await Models.Booking.find({ user: req.params.id }).populate('track')
...
module.exports = { createBooking,getAllBookings ,deleteBooking,getBookingsByUserId};
